...
export type GenerateFormat = 'claude' | 'cursor' | 'copilot' | 'windsurf' | 'antigravity' | 'all';
...
export function generateAll(store: Store, options: GenerateOptions): GenerateResult {
    ...
    if (format === 'windsurf' || format === 'all') {
        const targetPath = path.join(options.rootDir, '.windsurfrules');
        if (!options.force && fs.existsSync(targetPath) && !isAutoGenerated(targetPath)) {
            const existing = fs.readFileSync(targetPath, 'utf-8');
            const instructions = renderAtlasInstructions(data);
            if (existing.includes('AtlasMemory — AI Agent Protocol')) {
            ...
            results.files.push({
                path: targetPath,
                content: renderCursorRules(data).replace('Re-run `atlasmemory generate --format cursor`', 'Re-run `atlasmemory generate --format windsurf`'),
            });
        }
    }

    if (format === 'antigravity' || format === 'all') {
        const agentsDir = path.join(options.rootDir, '.agents');
        const targetPath = path.join(agentsDir, 'AGENTS.md');
        if (!options.force && fs.existsSync(targetPath) && !isAutoGenerated(targetPath)) {
            const existing = fs.readFileSync(targetPath, 'utf-8');
            const instructions = renderAtlasInstructions(data);
            if (existing.includes('AtlasMemory — AI Agent Protocol')) {
            ...
            results.files.push({
                path: targetPath,
                content: renderCopilotInstructions(data).replace('Re-run `atlasmemory generate --format copilot`', 'Re-run `atlasmemory generate --format antigravity`'),
            });
        }
    }
