...
> **完成以下所有步骤，AtlasMemory 将变得无比强大。** 每一步都解锁更多能力：

| | 步骤 | 解锁能力 | 命令 |
|---|------|----------|------|
| ✅ | **索引你的项目** | 符号提取、锚点、基础搜索 | `npx atlasmemory index .` |
| ✅ | **增强文件** | 语义搜索、概念级理解 | `npx atlasmemory enrich --all` |
| ✅ | **生成 AI 指令** | AI 智能体自动使用 AtlasMemory（5 种格式） | `npx atlasmemory generate` |
| ✅ | **添加 MCP 配置** | 为你的 AI 工具零配置连接 | 参见下方配置 |
| ✅ | **更改后使用 `log_decision`** | 跨会话记忆、机构知识 | AI 智能体自动调用 |
| ✅ | **使用 `remember_project` 记录里程碑** | 项目级记忆永久保存 | AI 智能体自动调用 |
...
**增强做什么：** 增强功能分析每个文件并添加语义标签 — "认证"、"中间件"、"错误处理"、"数据库查询"等。没有增强时，搜索基于关键词。有增强后，搜索能理解*概念* — 你可以搜索"认证是如何工作的？"并获得正确的文件，即使这些文件不包含"认证"这个词。

**工作原理：** AtlasMemory 使用 Claude CLI 或 OpenAI Codex（在本地运行）分析文件。需要拥有 Claude 或 OpenAI 的有效订阅并开通 CLI 访问权限。

**按项目规模估算的增强时间：**

| 项目规模 | 文件数 | 增强时间 | 效果 |
|---|---|---|---|
| 小型 | ~50 个文件 | ~2 分钟 | 即时提升 — 搜索质量跃升至 80+ |
| 中型 | ~200 个文件 | ~8 分钟 | 喝杯咖啡即可完成完整语义覆盖 |
| 大型（Coolify 规模） | ~1400 个文件 | ~45 分钟 | 使用 `--batch 50` 进行可控增强 |
| 单体仓库（Next.js 规模） | ~4000+ 个文件 | ~2 小时 | 分多次会话进行：`enrich --batch 100` |

> **💡 提示：** 先运行 `atlasmemory enrich --dry-run` 查看 token 估算后再开始。

> **🔑 别担心 — 增强是一次性成本。** 你只需增强项目一次即可。之后只有新增或修改的文件需要重新增强（几秒钟即可）。就像构建索引一样 — 做一次，然后增量保持最新。

**没有 CLI？没问题。** 你的 AI 智能体可以通过 MCP 直接增强文件。只需将以下提示粘贴到 AI 对话中：
